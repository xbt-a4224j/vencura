...
describe('PolicyEngine', () => {
  ...
  it('denies a recipient not on a non-empty allowlist', async () => {
    prismaMock.walletPolicy.findUnique.mockResolvedValue({ allowlist: ['0xOther'], perTxLimit: null, dailyLimit: null });
  ...
  it('denies a native amount over the per-tx limit', async () => {
    prismaMock.walletPolicy.findUnique.mockResolvedValue({ allowlist: [], perTxLimit: '500', dailyLimit: null });
    await expect(engine.assertAllowed('w1', send)).rejects.toBeInstanceOf(ForbiddenException);
  });
  ...
  it('allows within limits and allowlist', async () => {
