...
export class AllExceptionsFilter implements ExceptionFilter {
  private readonly logger = new Logger(AllExceptionsFilter.name);
    ...
    if (body.status >= HttpStatus.INTERNAL_SERVER_ERROR) {
      // Log full detail server-side only; the client gets the generic body.
      this.logger.error(`${body.status} ${body.title}: ${(exception as Error)?.message ?? exception}`);
    }
  ...
  private shape(exception: unknown): ProblemDetails {
    if (exception instanceof HttpException) {
      ...
      return { type: 'about:blank', title: this.titleFor(status), status, detail: detailOf(exception) };
    }
    const mapped = mapChainError(exception);
    if (mapped) {
      return { type: 'about:blank', title: this.titleFor(mapped.status), status: mapped.status, detail: mapped.detail };
    }
    const status = HttpStatus.INTERNAL_SERVER_ERROR;
    return { type: 'about:blank', title: this.titleFor(status), status, detail: 'Internal server error' };
  }
