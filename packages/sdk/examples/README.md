...
Runnable scripts that show how to drive the VenCura API with the typed
[`VencuraClient`](../src/index.ts). Each is self-contained and readable top to bottom.
...
| `04-send-transaction.ts` | send native ETH between wallets, poll to confirmed (idempotency key) |
