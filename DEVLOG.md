...
## v0.x.0 · Block 5 · Clickable logo + admin/user separation + rename
**What & why** — (1) The in-app header logo (◆ VenCura) now links Home. (2) The seeded operator is renamed `demo@vencura.local` → `admin@vencura.local` (clarity: it's an admin, not a user). (3) Fixed the real "admin shows up as a user" bug — it was a *shared-session* bleed (the Admin view's session leaked into the User view), not a data leak. `/auth/user` already filters `isDemo`.
**How it works** — Shared `ADMIN_EMAIL` constant (one source of truth). [UserView](packages/web/src/App.tsx) renders UserAuth when `current` is the admin (admin never counts as the user); [AdminView] always (re)signs-in as the admin even if a stale user session exists. Seed/auth/provisioning use `ADMIN_EMAIL`. Logo = a `.logobtn` inside the h1 → `onExit`.
**Files** — shared/auth.schema (ADMIN_EMAIL), web/api.ts (mirror), App.tsx (guards + logo), seed.ts, auth.service.ts, provisioning.service.ts, sdk examples; index.css (.logobtn).
**Tests** — api 91 pass; web typecheck/lint/build green.
**Gotchas** — Needs a prod reset to rename the existing `demo@vencura.local` row → `admin@vencura.local` (reset wipes users + reseeds; demo_token survives). The historical migration's `WHERE email='demo@vencura.local'` backfill is left as-is (already applied).
